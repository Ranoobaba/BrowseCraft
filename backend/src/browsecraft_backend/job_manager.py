...
from .imagine_service import ANTHROPIC_VISION_MODEL, GEMINI_IMAGE_MODEL, ImaginePipeline
from .models import (
    BuildJobCreated,
    BuildRequest,
    EventEnvelope,
        ...
        async with self._job_semaphore:
            with laminar_span(
                ...
                try:
                    ...
                    with laminar_span(
                        "job.download_parse",
                        payload={"job_id": state.job_id, "candidate_count": len(candidates)},
                    ):
        ...
        async with self._job_semaphore:
            with laminar_span(
                ...
                try:
                    ...
                    plan = await self._imagine_service.build_plan(state.query)
                    ...
                    state.source = SourceInfo(
                        type="imagine",
                        url=f"imagine://{GEMINI_IMAGE_MODEL}+{ANTHROPIC_VISION_MODEL}",
                    )
